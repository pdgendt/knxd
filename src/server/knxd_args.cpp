...
static IniData ini;
static char link_buf[99] = "@.";
static void link_to(const char *arg)
{
  char *p;
  ++*link_buf;
  strcpy(link_buf+2,arg);
  p = strchr(link_buf+2,':');
  if (p)
    *p = 0;
}
    ...
    if (more_args.size() > 0)
      {
        link_to(name);
        ITER(i, more_args)
        (*ini[link_buf])[i->first] = i->second;
        (*ini[link_buf])["filter"] = name;
        more_args.clear();
        filters.push_back(link_buf);
      }
    else
...
parse_opt (int key, char *arg, struct argp_state *state)
{
  ...
  switch (key)
    ...
    case 'u':
    {
      ...
      link_to("unix");
      ADD((*ini["main"])["connections"], link_buf);
      (*ini[link_buf])["server"] = "knxd_unix";
      // (*ini[link_buf])["driver"] = "knx-link";
      const char *name = OPT_ARG(arg,state,NULL);
      if (name)
        {
          (*ini[link_buf])["path"] = name;
          (*ini[link_buf])["systemd-ignore"] = "false";
        }
      else
        (*ini[link_buf])["systemd-ignore"] = "true";
      arguments->stack(link_buf);
    }
    break;
    ...
    case 'i':
    {
      ...
      link_to("tcp");
      ADD((*ini["main"])["connections"], link_buf);
      (*ini[link_buf])["server"] = "knxd_tcp";
      // (*ini[link_buf])["driver"] = "knx-link";
      const char *port = OPT_ARG(arg,state,"");
      if (*port && atoi(port) > 0)
        {
          (*ini[link_buf])["port"] = port;
          (*ini[link_buf])["systemd-ignore"] = "false";
        }
      else
        (*ini[link_buf])["systemd-ignore"] = "true";
